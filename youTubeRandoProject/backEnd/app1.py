...
CORS(app) # <-- ADD THIS to allow your frontend to talk to this API
...
def search():
    # This matches your frontend's request for query (q) and target time
    ...
    # This calls your logic from youTubeRandomizer.py
    video, error = find_random_video(query, target_minutes=target_minutes)

    if error:
        return jsonify({'error': error}), 404

    # This 'jsonify' sends the video data back to your JavaScript fetch()
    return jsonify({
        'title': video['title'],
...
if __name__ == '__main__':
    # You can specify the port here to be sure
